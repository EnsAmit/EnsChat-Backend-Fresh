...
class Environment {
    getEnvironmentString(key) {
      return process.env[String(key)] || '';
    }
  
    getEnvironmentNumber(key) {
      return Number(process.env[String(key)]) || 0;
    }
  
    getEnvironmentBoolean(key) {
      return Boolean(process.env[String(key)]);
    }
  
    getEnvironmentObject(key) {
      try {
...
const GlobalConfig = {
  ...
  APP_ROUTE: environment.getEnvironmentString('APP_ROUTE'),
